...
class ResidualUnit(object):
    ...
    def __init__(self, n_filters_out, n_samples_out, kernel_initializer='he_normal',
...
def get_model(n_classes, last_layer='sigmoid'):
    ...
    signal = Input(shape=(X_test.shape[1:]), name='signal')
    x = signal
    x = Conv1D(64, kernel_size, padding='same', use_bias=False,
               kernel_initializer=kernel_initializer)(x)
    x = BatchNormalization()(x)
    x = Activation('relu')(x)
    x, y = ResidualUnit(128, 999, kernel_size=kernel_size,
...
N_EPOCH = 70
m = get_model(len(Y_train[0]))
m.summary()
#keras.utils.plot_model(m, 'train/model4_conv.png', show_shapes=True)
